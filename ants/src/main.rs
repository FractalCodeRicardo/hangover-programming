...
impl Ant {
    ...
    fn new() -> Self {
        ...
        let dir = Direction::new(Ant::random_binary(), Ant::random_binary());
    ...
    fn mov(&mut self) {
        ...
        nx = nx.min(SIZE as f32);
        ny = ny.min(SIZE as f32);
...
impl Square {
    ...
    fn draw(&self) {
        let color = if self.color == 1 {WHITE} else {GRAY};
    ...
    fn change_color(& mut self) {
        self.color = if self.color == 1 {0} else {1}
    }
}
...
impl Game {
    ...
    fn mov_ants(&mut self) {
        for ant in &mut self.ants {
            ...
            let square = &mut self.squares[j * SIZE + i];
